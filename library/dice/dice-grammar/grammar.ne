...
DM -> DR "kl" N {% (d) => DiceModifier('kl', d[0], d[2]) %}
    | DR "kh" N {% (d) => DiceModifier('kh', d[0], d[2]) %}
    | DR "k" N {% (d) => DiceModifier('kh', d[0], d[2]) %}
    | DR "dl" N {% (d) => DiceModifier('dl', d[0], d[2]) %}
    | DR "dh" N {% (d) => DiceModifier('dh', d[0], d[2]) %}
    | DR "!" {% (d) => DiceModifier('!', d[0], '=', null) %}
    | DR "!" N {% (d) => DiceModifier('!', d[0], '=', d[2]) %}
    | DR "!>" N {% (d) => DiceModifier('!', d[0], '>', d[2]) %}
    | DR "!<" N {% (d) => DiceModifier('!', d[0], '<', d[2]) %}
    | DR "!=" N {% (d) => DiceModifier('!', d[0], '=', d[2]) %}
    | DR {% id %}
